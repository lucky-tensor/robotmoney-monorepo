...
| `watchdog-rate-monitor` | `feature-correctness` (`watchdog-unit`) / `system-correctness` (`watchdog-integration`) | Code-bearing PRs to any branch and pushes to `dev`/`dev-phase-*`; unit tests run on drafts, while Postgres-backed integration tests begin at `ready_for_review`; docs-only changes are skipped. The integration job also runs `cursor_and_volume` — the cursor-staleness and deposit-volume-anomaly suite, dark until issue #1282 (issue #1384) |
